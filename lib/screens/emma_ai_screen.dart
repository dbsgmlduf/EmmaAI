...
class _EmmaAIScreenState extends State<EmmaAIScreen> {
  ...
  Future<void> analyzeImage() async {
    ...
    try {
      ...
      if (context.mounted) {
        Navigator.pop(context); // 로딩 다이얼로그 닫기
        showCommonDialog(context, '오류', '이미지 분석 중 오가 발생했습니다.\n$e');
      }
    }
  }
  ...
  Future<void> saveChart(Map<String, dynamic> chartData) async {
    if (selectedPatient != null) {
      // findings와 painLevel 데이터 추가
      final completeChartData = {
        ...chartData,
        'findings': chart['findings'] ?? '',  // 현재 선택된 finding 값
        'painLevel': chart['painLevel'] ?? '5.0',  // 현재 설정된 pain level
      };
